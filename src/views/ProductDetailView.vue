    ...
    <div v-if="productStore.loading">Loading ...... </div>
      ...
      <template v-else-if="product">
          <section class="product-section">
            ...
            <img v-for="image in selectedVariant.images" :key="image" :src="image" :alt="product.name"/>
          </div>
             </div>
             <div class="product-info">
             <h1>{{ product.name }}</h1>
             <p class="description">{{ product.description }} </p>
             <div v-for="(values,attributeName) in product.attributes"
             :key="attributeName"
             class="attribute">
             <h3>
              {{ formatAttributeName(
            ...
            <div class="attribute-options">
              <button v-for="value in values" :key="value" class="attribute-button" :class="{
             ...
             <div> ⭐{{ selectedMerchant.rating }}</div>
               ...
               Stock: {{ selectedMerchant.stock }}</p>
             </div>

   <div class="actions">
        <button :disabled="!selectedMerchant || selectedMerchant.stock <= 0" @click="addToCart">Add to Cart</button>
        <button :disabled=" !selectedMerchant || selectedMerchant.stock <= 0" @click="buyNow">Buy Now</button>
          </div>
             </div>
          </section>
      ...
      <div v-if="!merchants.length" class="no-merchants">
          No sellers available for this
          variant.
        </div>
    
    
        <div v-for="merchant in merchants" :key="merchant.merchantId" class="merchant-card" 
        :class="{selected:selectedMerchant?.merchantId ===merchant.merchantId}"
        ...
        <div class="merchant-info">
         ...
         <div class="merchant-price">
              <strong>₹{{formatPrice(merchant.price)}}
            </strong>
         </div>
        </div>
    </section>
...
const response =await productStore.loadProduct(productId, merchantId);
 Object.keys(
    response.selectedVariant
      ...
      selectedAttributes[attributeName] = response.selectedVariant.attributes[attributeName];},);
}
...
async function changeAttribute(attributeName,value){
  ...
  try {
    const response=await productStore.changeVariant(
...
async function selectMerchant(merchant){
  ...
  await router.replace({
    ...
    query:{
    ...route.query,merchant:merchant.merchantId,
    }
  });
}
